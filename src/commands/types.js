 ...
 *   width/strokeWidth → CSS pixel integer
 *
 * Every shape carries a unique string `id`.
 * The LLM is instructed to use short, semantic English ids (sun, house_wall…).
 *
 * @typedef {{ type:'circle', id:string, x:number, y:number, r:number, fill?:string, stroke?:string, strokeWidth?:number, opacity?:number, groupId?:string, groupLabel?:string, role?:string }} CircleCmd
 * @typedef {{ type:'rect',   id:string, x:number, y:number, w:number, h:number, fill?:string, stroke?:string, strokeWidth?:number, opacity?:number, groupId?:string, groupLabel?:string, role?:string }} RectCmd
 * @typedef {{ type:'line',   id:string, x1:number, y1:number, x2:number, y2:number, stroke?:string, width?:number, strokeWidth?:number, opacity?:number, groupId?:string, groupLabel?:string, role?:string }} LineCmd
 * @typedef {{ type:'text',   id:string, x:number, y:number, content:string, size?:number, fill?:string, opacity?:number, groupId?:string, groupLabel?:string, role?:string }} TextCmd
